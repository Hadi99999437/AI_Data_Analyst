...
class AnalysisService:
    ...
    async def run_analysis(
        ...
        try:
            ...
            # Histogram(s)
            charts["histograms"] = (
                visualization_service.create_histogram(df)
            )

            # Correlation Heatmap
            heatmap = visualization_service.create_heatmap(df)
            charts["heatmap"] = heatmap

            # Boxplots
            charts["boxplots"] = (
                visualization_service.create_boxplots(df)
            )

            # Missing Values Chart
            missing_chart = (
                visualization_service.create_missing_values_chart(df)
            )
            charts["missing_values"] = missing_chart

            # Bar Charts
            charts["bar_charts"] = (
                visualization_service.create_bar_charts(df)
            )

            # Scatter Plots
            charts["scatter_plots"] = (
                visualization_service.create_scatter_plots(df)
            )
